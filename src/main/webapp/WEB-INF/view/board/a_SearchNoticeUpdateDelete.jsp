...
function del() {
	...
	if(confirm("공지를 삭제하시겠습니까?") == true) {
		...
		location.href='searchNoticeDel?owner_Notice_Num=' + num + '&&searchstr=${searchstr}&&search=${search}';
		alert("삭제되었습니다.");
		return true;
			}
	}
	...
	function ChkCnt() {
		...
		var files = $('input[name=files]').val().text();
...
<form id="searchNoticeUpdate" action="searchNoticeUpdate" method="post" onsubmit="return chk()" >
	...
	<b> 제목 :</b> <input type="text" id="owner_Notice_Title" name="owner_Notice_Title" style="width: 400px;"value="${choose.owner_Notice_Title}">
	...
	value="삭제" onclick="del()">
 	<input type="button" style="background-color: #665F79;float: right;margin-left: 4px;
	color: white;border-radius: 5px 5px 5px 5px;padding-left: 10px;padding-right: 10px;"
	value="취소" onclick="location.href='goSearchNoticeRead?searchstr=${searchstr}&&noticeNum=${choose.owner_Notice_Num}&&search=${search}'">
 	</p>
 	</table>
 </form>
 </div>
 ...
 <!-- 파일업로드 input-->
 <form id="searchNoticeUpdate" name="searchNoticeUpdate" action="searchNoticeUpdate" enctype="multipart/form-data" onsubmit="return ChkCnt()" method="post">
