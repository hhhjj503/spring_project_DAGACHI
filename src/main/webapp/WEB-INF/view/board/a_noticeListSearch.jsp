...
<c:if test="${log_In_Admin != null }" > 
	...
	<b>${log_In_Admin.admin_Name}</b> 님 안녕하세요
...
<form action="a_noticeListSearch" method="post">
	<table border="1" align="center" id="listtable"
			...
			onclick="location.href='a_noticeList'" value="공지사항 이동">
			</td>
			...
			<th style="border: none;height: 6px;padding-left: 10px;padding-right: 10px;" >수정/삭제</th>
			</tr>
			</thead>  
	...
	<c:forEach items="${searchList.ownerList}" var="result" varStatus="status">
		...
		<c:if test="${searchList.count > 0}">
  		...
  		<c:if test="${searchList.p.beginPageNumber > 10}">
			<a href="a_noticeListSearch?p=${searchList.p.beginPageNumber-1}&&searchstr=${searchstr}&&search=${search}">이전</a>
		</c:if>
		
		<c:forEach var="pno" begin="${searchList.p.beginPageNumber}" end="${searchList.p.endPageNumber}">
		<a href="a_noticeListSearch?p=${pno}&&searchstr=${searchstr}&&search=${search}">[${pno}]</a>
		</c:forEach>
		
		<c:if test="${searchList.p.endPageNumber < searchList.p.totalPageCount}">
			<a href="a_noticeListSearch?p=${searchList.p.endPageNumber + 1}&&searchstr=${searchstr}&&search=${search}">다음</a>
		</c:if>
